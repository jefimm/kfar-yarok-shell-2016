...
private:
	...
	long size;
public:
	FileInfo(const string& dirname) : name(dirname), size(-1) { };
	FileInfo(const string& filename, long filesize) : name(filename), size(filesize) {};

	inline long getsize() const { return size; }
	inline const string& getname() const { return name; }

	inline bool isDir() const { return size == -1; }
};
...
cerr
*/
void get_directory_listing(const string &dir, vector<FileInfo>& out){
		WIN32_FIND_DATA ffd;
		...
		do
		{
			...
			else
			{
				...
				out.push_back(FileInfo(filename, filesize.QuadPart));
